...
class NotificationManager {
  ...
  aim_receive() {
    this._playTone([440, 554, 659, 880], [0.06, 0.06, 0.06, 0.12], 'sine');
    setTimeout(() => this._playTone([880, 1047], [0.04, 0.08], 'sine'), 200);
  }
  aim_send() {
    this._playTone([659, 784, 880], [0.05, 0.05, 0.1], 'sine');
  }
  flip_sms() {
    this._playTone([1319, 988, 1319], [0.04, 0.04, 0.06], 'square');
    setTimeout(() => this._playTone([1319, 988, 1319], [0.04, 0.04, 0.06], 'square'), 180);
  }
  flip_ring() {
    const playBurst = () => this._playTone([1397, 1568, 1397, 1568, 1397], [0.06, 0.06, 0.06, 0.06, 0.06], 'square');
    playBurst();
    setTimeout(playBurst, 400);
    setTimeout(playBurst, 800);
  }
  dm_notify() {
    this._playTone([523, 784, 1047], [0.08, 0.08, 0.15], 'sine');
  }
