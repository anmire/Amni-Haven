...
class HavenApp {
  ...
  _renderOnlineUsers(users) {
    ...
    if (sidebarEl) sidebarEl.innerHTML = onlineUsers.map(u => `<div class="user-item mini" data-user-id="${u.id}"><span class="user-dot"></span><span class="user-item-name">${this._escapeHtml(u.displayName || u.username)}</span></div>`).join('') || '<p class="muted-text">No one online</p>';
    if (sidebarCount) sidebarCount.textContent = onlineUsers.length;
  }
  ...
  async _setupGameUI() {
    ...
    if (html5Grid) {
      html5Grid.innerHTML = this.gameManager.getHTML5Games().map(g =>
        `<div class="html5-game-card" data-game-id="${g.id}" title="${g.desc}"><span class="html5-game-icon">${g.icon}</span><span class="html5-game-name">${g.name}</span><span class="html5-game-players">${g.maxPlayers > 1 ? g.maxPlayers + 'P' : '1P'}</span></div>`
      ).join('');
      html5Grid.addEventListener('click', (e) => {
        ...
        try {
          ...
          const gm = this.gameManager.getHTML5Games().find(g => g.id === gid);
