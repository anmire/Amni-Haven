...
:: The server auto-creates .env on first run, but let users know where it is
if not exist "%HAVEN_DATA%\.env" (
    echo  [*] Config file will be created on first launch at:
    echo      %HAVEN_DATA%\.env
) else (
    echo  [OK] Config file exists: %HAVEN_DATA%\.env
)
echo.
...
:: Use PowerShell to elevate and add firewall rule
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo      You can add it manually later or allow it when Windows prompts you.
)
...
echo.
echo    [OK] Node.js
echo    [OK] Haven dependencies (node_modules)
echo    [OK] Data directory: %HAVEN_DATA%
echo    [OK] Desktop shortcut: "Start Haven"
echo.
echo  Next steps:
echo.
...
echo.
echo    3. Register your admin account (first user = admin)
echo.
echo    4. Create a channel and invite your friends!
echo.
echo  Need help? See GUIDE.md in this folder or visit:
echo    https://github.com/ancsemi/Haven
echo.
echo  --------------------------------------------------------
echo.
